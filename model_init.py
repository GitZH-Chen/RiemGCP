...
class Newmodel(Basemodel):
    ...
    def __init__(self, modeltype, representation, num_classes, freezed_layer, pretrained=False):
        ...
        self.is_bias=True
        if 'seperate_P' in representation:
            self.is_bias = not representation['seperate_P']
        if representation is not None:
            ...
            if not pretrained:
                if isinstance(self.classifier, nn.Sequential): # for alexnet and vgg*
                    ...
                    self.construct_classifier(fc_input_dim,num_classes)
            else:
                self.construct_classifier(fc_input_dim,num_classes)
        else:
    ...
    def construct_classifier(self,fc_input_dim,num_classes):
            self.classifier = nn.Linear(fc_input_dim, num_classes,bias=self.is_bias)
