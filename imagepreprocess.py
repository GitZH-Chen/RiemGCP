...
class NormalizeCrops:
    ...
    def __call__(self, crops):
        return torch.stack([self.normalize_fn(crop) for crop in crops])
...
def preprocess_strategy(dataset):
    ...
    if dataset.startswith('CUB'):
        ...
        #     CenterCropWithFlip(448),
        #     StackCropsTensor(),
        #     NormalizeCrops(normalize),
        # ])
    elif dataset.startswith('fgvc-aircraft') or dataset.startswith('Dogs'):
        train_transforms = transforms.Compose([
        ...
        #     NormalizeCrops(normalize),
        # ])
    elif dataset.startswith('fgvc-cars'):
        train_transforms = transforms.Compose([
